...
**最新研究：** [空间隔离候选与时钟曲率检验](experiments/sequestering_test_v01/README.md)已把一个有原始来源的五维有限引力系数接入条件模型：带电接触可以算得很小，但同号时钟度规扩展会产生横向负曲率。$m_G=1$ eV、$m_{\rm KK}=1$ TeV 的已选带电反馈仅约0.422%，仍未通过横向局部检验；同一KK尺度下的 $m_G=10^{-18}$ eV 示例通过所列必要条件。后者不等于完整UV完成或滚动宇宙稳定。下一步明确时钟与带电场的边界布置、横向稳定来源、同边界接触及完整两环匹配；[86页正式稿](manuscript_cn/paper.pdf)保持原版。
