...
当前写作方向：**按原文第0—9章构建中文逐章修订稿，已完成摘要、第0章介绍与两张总览图**，入口见[manuscript_cn](manuscript_cn/README.md)。此前摘要、七章正文及三个附录的完整稿作为技术材料保留；其结构不替代原文十章。计算已完成至真实零点残差与替代输入的条件比较v0.5。写作更新日期为2026-09-23；最近一轮计算完成于2026-09-22。当前成果是明确的条件模型与可复现计算，尚未获得常数演化的观测确认。
...
- **[按原文逐章修订：中文累计稿](manuscript_cn/paper.md)**／[PDF（11页）](manuscript_cn/paper.pdf)：已完成摘要、[第0章介绍](manuscript_cn/chapters/00_introduction.md)、两张总览图和当前参考表。[章节对应表](manuscript_cn/README.md)与[第0章逐项对应](manuscript_cn/notes/chapter00_alignment.md)说明原I—VIII结构如何融入新内容。下一步进入第1章。
