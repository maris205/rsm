...
**当前主线与收口范围（2026-09-26，按用户要求收敛）：** 从同一个自主非线性格点与内部时钟 Hamilton 量，说明怎样得到宏观长波场方程、区域能量流与总能量守恒、连续场动量守恒，以及指定逆对数平方响应在有限格点正能量分支中的条件晚时律。核心命题、证据与假设集中见[微观—宏观核心结论](reports/micro_macro_core_20260926.md)。三维各向同性无质量标量波的 $p=\rho/3$ 作为有附加条件的推论；这些计算没有导出光子、引力或真实常数的演化，也没有唯一选择平方指数。

**本版已收口（2026-09-26）：** 核心命题与证据已并入第4章，摘要及第9章结论已对齐；完成一次证据、条件与措辞的一致性审阅，并重新导出[86页单栏整稿](manuscript_cn/paper.pdf)及[三页摘要与图版](manuscript_cn/front_matter.pdf)。原章次、19幅图、63条参考及历史失败记录保留，新增自含哈密顿量与三档完整反馈收敛表。过程与版本身份见[收口修订记录](reports/manuscript_convergence_20260926.md)。本轮完成约定写作与导出，不新增科学实验；新场、完整引力或真实常数接口留作后续独立研究。

Higgs链、轻模稳定和超引力匹配保留为独立的物质接口研究。其具体候选遇到的障碍限制相应接口，不否定已定义自主标量时钟模型的条件推导；实方向稳定仍是该接口的开放问题，不再占据当前写作流程。下列阶段中的“下一步”记录当时计划，当前优先级以上述收口路线为准。

**物质接口阶段记录（2026-09-26，保留轻模与完整驻点）：** [light_modulus_v01](experiments/light_modulus_v01/README.md)已将原链未被吸收的整个手征模保留，推导其准确约束商空间动能及两端局域接入。$q=3,n=127$ 时，左端零模分量约为 $2.399\times10^{-61}$，右端零模分量却约为 $-0.942809$；小端点传递不能使右端对轻模的受力自动消失。在非零隐藏源、正定动能、$X=Z_a=0$、$W_{\rm clock}=0$ 的声明刚性分支中，**没有有限内部驻点**；保留有限径向响应的核验也支持这一限定结论。旧固定轻坐标的正曲率不再足以判断真空稳定，相位仍保持平坦。[结果报告](experiments/light_modulus_v01/reports/results_cn.md)与[诊断图](experiments/light_modulus_v01/figures/light_modulus_diagnostics.png)记录推导及失败条件。该接口当时提出从明确微观作用加入保持相位对称性的实方向稳定来源，同算谱、物理流、接触及慢时钟反馈；相位可继续保留为轻场。完整超引力和圈匹配仍待完成，$1/\ln^2t$ 及真实相互作用的黎曼来源尚未导出；86页正式稿及历史实验保持原版。

**上一阶段研究（2026-09-26，Higgs剩余模与局域流）：** [higgs_stabilization_v01](experiments/higgs_stabilization_v01/README.md)检验了两种明确的稳定路线。原场内容的相位提升需要最低 $D\simeq5.89509\times10^{60}$ 次多项式，在受控截断以下不能把剩余模变重。额外增加一个仅作用于右端Higgs对的 $U(1)$ 可以消去该连续平坦模，并获得全重的刚性超对称自由谱；但实际局域端点流的领先交叉接触**恰好为零**，所需持续恢复机制随之失去原有来源。右端自作用仍须保留。独立有限辅助场计算确认，在声明的无时钟F项刚性驻点支中，新增静态时钟恢复势仍为零；这不是全部超引力或辅助场效应的消失定理。[结果报告](experiments/higgs_stabilization_v01/reports/results_cn.md)与[比较图](experiments/higgs_stabilization_v01/figures/stabilization_tradeoff.png)保留两条路线的具体障碍。**该阶段提出保留原轻模并计算其势和耦合；上列轻模实验已完成此项限定检验，实方向稳定保留为该接口的开放问题，不预先要求破坏相位对称性。** 本轮未建立完整物理模型，也未排除其他完成方式；逆对数平方及真实相互作用的黎曼来源仍待推导，86页正式稿及历史实验保持原版。

**早前研究（2026-09-26，阈值与链保护）：** [threshold_rg_v01](experiments/threshold_rg_v01/README.md)已在固定旧有限边界下完成所选刚性重／轻部门的一圈阈值匹配：大对数移入运行系数，物理反馈不因改计算尺度而消失。2 TeV、127站点、$m_G=10^{-15}$ eV 例子的新增树加所选一圈反馈为原驱动力的 $2.00086\times10^{-5}$，即 **0.00200086%**。圈／新增树修正仍为1.234，但实际轻场顶点的大对数展开参数约 $3.63\times10^{-23}$，不能把前一比值等同于后续圈失控。保持所选交叉耦合量级的条件也已量化：指定端点泄漏与质量旁路需分别控制在约 $10^{-61}$、$10^{-60}$；这不是已计算出的辐射下限。具体矢量型Higgs链能产生目标质量矩阵，却留下一个未被矢量吸收的平坦手征模，已有体模量的远端接入仍无保护。**该阶段提出在此场内容中稳定剩余模、计算实际诱导耦合与旁路，再做完整局部真空匹配；上列Higgs实验已检验其中两条路线。** [结果报告](experiments/threshold_rg_v01/reports/results_cn.md)区分这些已算条件与完整量子模型；[86页正式稿](manuscript_cn/paper.pdf)及历史实验保持原版。

**早前研究（2026-09-26，中介链）：** [有能隙的中介链与完整反馈](experiments/mediator_chain_v01/README.md)已把极小端点耦合变为声明链内可计算的树级结果：$q=3,n=128$ 给出 $\tau\simeq2.26177\times10^{-61}$。但同一匹配必然保留系数为1的模量四次自作用，原 $m_G=10^{-6}$ eV、$\Lambda=1$ TeV 例子的静态反馈约为慢驱动力的 $1.43\times10^{23}$ 倍，完整局部超引力检查确认没有被原有抵消结构消去。2 TeV、127站点、$m_G=10^{-15}$ eV 的对照有较小静态响应，但所选刚性一圈／树响应约1.23，大对数尚需分阈值匹配与RG处理。**链的小系数来源推进了，量子保护和完整模型尚未成立。** 近邻结构、体模量只耦合远端及独立接触项的控制仍需具体场内容；下一步优先处理阈值/RG、局域性来源和完整真空重匹配。[结果报告](experiments/mediator_chain_v01/reports/results_cn.md)保留全部失败与限定范围；[86页正式稿](manuscript_cn/paper.pdf)和历史实验保持原版。

**早前研究（2026-09-26，持续恢复力）：** [持续恢复力与相关物质反馈](experiments/persistent_restoration_v01/README.md)已完成一个明确四维共享流候选：已有模量的非零辅助场使横向正质量在 $U=0$ 时仍可保留，同来源物质软项也一并计算。主例 $m_G=10^{-6}$ eV、$M_V=\Lambda=1$ TeV、$m_{\rm KK}=100$ TeV、$r_H=4,\sigma=1$ 的有限区间已选反馈约0.0253%；$U=0$ 的正质量平方约 $1.04063\times10^{-41}$ eV²，所选完整带电圈修正未翻转其符号。**所需 $\tau\simeq2.92515\times10^{-61}$ 仍是未经解释或保护的输入。** 持续软项留下的量子斜率会在固定外推中超过指数衰减的树驱动力，相关χ根不是宇宙日期；$r_H=2$ 的首阶相消也不是已建立的保护机制。下一步固定小τ的具体来源，计算未消元重场完整谱及圈匹配，同时检验持续质量和净量子斜率。[86页正式稿](manuscript_cn/paper.pdf)及历史实验保持原版。

**早前研究（2026-09-26，共享边界稳定）：** [共享边界中介与时钟稳定](experiments/clock_stabilization_v01/README.md)从一个明确的局部 Stückelberg 矢量候选，同时得到横向恢复项 $12U/\Lambda^2$ 与带电软项 $2\sigma U/\Lambda^2$。在 $m_G=10^{-6}$ eV、$M_V=\Lambda=1$ TeV、$m_{\rm KK}=100$ TeV、$M_Q=100$ GeV、$\sigma=1$ 的例子里，旧有限χ区间通过所列局部曲率、层次及已选反馈检查，反馈约0.0253%。**恢复力随 $U\to0$ 消失，尚未得到永久稳定真空。** 固定参数外推的 $\chi_{\rm crit}\simeq143.2279$ 只是坐标标志，不是宇宙日期。下一步研究持续恢复力的来源或 $A,m_G$ 的自洽演化，并完成重中介的谱与匹配；完整五维及全部圈修正仍未完成。[86页正式稿](manuscript_cn/paper.pdf)保持原版。

**中文完整初稿已完成：摘要、第0—9章、参考文献、致谢与研究材料说明，含十九幅图及63条参考文献。** [单栏PDF（86页）](manuscript_cn/paper.pdf)与[章节入口](manuscript_cn/README.md)按原文十章组织。此前七章技术稿继续作为推导与计算来源保留。2026-09-24扩充第4章，新增局部能流、连续动量、条件波动压力、CMB传播与统计投影的推导，以及[可复算的代数和小格点核验](experiments/macro_laws_v01/README.md)。334项守恒/压力与28项波动/传递检查通过。随后补回CMB约束的n=0—95时间线，并提出含明确负压条件的暴涨候选；暴涨分支尚未执行背景/扰动积分。进一步新增[宇宙年龄与PM聚类实验](experiments/cosmic_bridge_v01/README.md)：正则时钟候选接入辐射、物质、Λ和标准引力，完成共同初态的8组后期聚类及分辨率对照；大尺度配对功率变化约−0.027%至−0.029%，属于示例模型的条件计算。另接入[受控塌缩与条件冷却](experiments/halo_cooling_v01/README.md)：连续球形方程及原初H/He接口通过独立核验，但三维主配置未达到预设连续精度，完整失败与对照均保留。2026-09-25完成[两档匹配粒子／力网格检验](experiments/pm_matched_scale_v01/reports/results_cn.md)：128/128事件误差降至0.317%、最大半径误差降至3.1266%，但后者仍超过3%门限，两档事件差也未过1%要求；角向误差并非全部改善。星系形成仍待气体和恒星过程。本轮没有重新拟合观测。当前整稿修订按上列有限收口路线推进，物理接口验证另行保留。当前成果是明确的条件模型及数值记录，尚未获得常数演化的观测确认。
...
现有论文与研究材料（物质接口按历史阶段保留）：

- **[保留Higgs轻模、完整驻点与端点响应](experiments/light_modulus_v01/README.md)**／[结果报告](experiments/light_modulus_v01/reports/results_cn.md)／[诊断图](experiments/light_modulus_v01/figures/light_modulus_diagnostics.png)：保留原轻手征模后，明确区分极小左接入与正常大小右接入。所选正定动能、非零隐藏源、$X=Z_a=0$、$W_{\rm clock}=0$ 刚性分支无有限内部驻点；固定轻坐标得到的正曲率不能替代完整真空判据。主549项、独立几何507项、驻点1435项（含216组完整振幅求解）及响应110项检查通过，属于内部核验。低源例 $\kappa\simeq5.00278\times10^{-24}$ 只说明所选重场响应很小，不能删去轻模的线性受力；$\beta_X^2\Lambda^2\simeq32$ 也未被端点指数压低。该接口后续可研究明确的、保持相位对称性的实方向稳定作用及同来源谱／接触匹配；完整超引力、圈修正与宇宙背景尚未完成，86页正文未改。
- **[Higgs剩余模稳定与局域端点匹配](experiments/higgs_stabilization_v01/README.md)**／[结果报告](experiments/higgs_stabilization_v01/reports/results_cn.md)：原场内容的最小相位敏感多项式次数约 $5.89509\times10^{60}$，受控截断以下的提升极弱。额外右端 $U(1)$ 能消去剩余连续平坦模；$\eta=2,\lambda/g=3,\Lambda=2$ TeV 的刚性自由谱最轻质量约1.193 TeV，通过原探针层次条件。但完整局域Higgs行流的树级交叉接触严格为零，自作用比仍为9，不能延用旧持续恢复项。非零抽象站点传递对应跨链接源及极小共享成分，其来源未获解释。有限辅助场另在无时钟F项的声明刚性支中直接核验，不能外推为全部超引力或高阶辅助场修正都消失。当时提出的轻模势与耦合任务已由上列轻模实验执行，保持相位对称性的实方向稳定仍是该接口的开放问题；没有新观测、宇宙积分或正式正文改动。
- **[阈值匹配、轻场圈计数与链保护](experiments/threshold_rg_v01/README.md)**／[结果报告](experiments/threshold_rg_v01/reports/results_cn.md)：固定高尺度额外有限反项后，重阈值、轻圈和运行系数给出一致的势与斜率；选择场依赖计算尺度须保留链式求导。低尺度例子新增树加所选一圈反馈约0.00200086%，实际轻场顶点的展开参数约 $3.63\times10^{-23}$；这仍未覆盖全部高尺度有限项或超引力圈。两种指定旁路的敏感度揭示极小传输所需保护，$r_H=2.31964$ 的主阶持续曲率仅容许约13.78%的向下变化。矢量型Higgs场内容给出目标质量矩阵，但尚有一个未稳定手征模及未保护的体模量耦合。主376项、独立推导105项、轻场圈计数67项与局域性249项检查通过，独立输出比较另列；这些是实现核验，不累加为物理证据。下一步稳定剩余模并计算实际诱导算符、完整局部真空与谱；没有新增观测、宇宙积分或正式正文改动。
- **[有能隙的中介链与完整反馈](experiments/mediator_chain_v01/README.md)**／[结果报告](experiments/mediator_chain_v01/reports/results_cn.md)：百余站点使树级端点交叉系数指数减小，完整匹配却同时产生未被指数压低的 $|X_c|^4/\Lambda^2$ 自作用。原能标例子发生巨大静态反馈，1 TeV主例也未通过探针／最轻中介0.1的谱隙要求。2 TeV、127站点、$m_G=10^{-15}$ eV 的选定静态反馈为 $8.95\times10^{-6}$，刚性一圈反馈为 $1.105\times10^{-5}$，两者各自较小而圈／树比约1.23，不能据此宣布逐圈稳定。主124项、几何978项、局部超引力359项、来源38项检查通过，另有独立输出比较；刚性谱检查为512/513，旧能标二阶近似斜率误差0.2478%超出预设0.2%，旧例采用精确谱结果并保留该失败。普通规范与超对称不自动保护近邻纹理、体模量端点接入或独立Kähler旁路。后续为具体场内容、分阈值匹配/RG与完整真空重匹配；没有新增观测、宇宙积分或正式正文改动。
- **[持续恢复力与相关物质反馈](experiments/persistent_restoration_v01/README.md)**／[结果报告](experiments/persistent_restoration_v01/reports/results_cn.md)：在共同中介流中加入已有模量的规范位移，得到持续部分 $\Delta d_Q=(\sigma/2)\Delta m_y^2$，并保留隐藏自稳定及多场混合。声明主例的 $U=0$ 横向质量为正，所选完整带电圈增量约为其 $-0.0261\%$，未翻转符号；旧区间反馈约0.0253%，但需要未有来源解释的 $\tau\simeq2.92515\times10^{-61}$。净常数软项的量子轴力呈代数尾，不能永久小于指定的指数树力；有限χ外推不对应宇宙日期。$r_H=2,\sigma=1$ 的首阶相消未控制二阶误差及已知真空软项，其形式远期根不作物理预测。主91项、独立1012项及27项输出比较通过，另列559项几何、377项带电谱和47项来源检查；范围重叠，不累加为物理证据。下一步为小耦合的明确保护来源、未消元重谱及完整圈匹配；没有新观测、宇宙积分或正式正文改动。
- **[共享边界中介与时钟稳定](experiments/clock_stabilization_v01/README.md)**／[结果报告](experiments/clock_stabilization_v01/reports/results_cn.md)：树级流平方匹配将稳定项及同边界物质接触联合固定，主例的已选反馈约0.0253%、$|F^T_{\rm can}|/M_V^2=0.00421754$；原1 eV示例没有被修复。有限区间通过不等于渐近稳定：$U=0$ 的参考驻点仍有横向负曲率。主117项、独立490项及覆盖5053行的21项比较通过；另有843项几何、62项补充审阅和25项来源检查，范围重叠，不累加为物理证据。本轮修正五维 $B$ 项相对于β的符号；旧阶段保存的绝对值预算和分类不受影响，历史文件保持原版。没有新宇宙积分或观测拟合，完整重多重态圈图及未来持续稳定仍待研究。
- **[空间隔离候选与时钟曲率检验](experiments/sequestering_test_v01/README.md)**／[结果报告](experiments/sequestering_test_v01/reports/results_cn.md)：导入五维规范边界引力部门的有限系数 $A=\zeta(3)m_{\rm KK}^2/(48\pi^4M_P^2)$，同时保留几何与物质度规修正，得到 $c_Q\simeq12A$。向移位时钟的同号度规扩展 $\kappa=1$ 是额外假设，并给出相关关系 $\Delta m_y^2\simeq-24Am_G^2=-2c_Qm_G^2$。在 $m_{\rm KK}=1$ TeV、$\nu_0=0$ 下，1 eV示例的选定受力比为 $4.22335\times10^{-3}$，却有对应约20.4秒局部线性增长尺度的负曲率；$10^{-18}$ eV示例通过所列必要局部条件，尚未证明全局或宇宙滚动稳定。主177项、独立推导153项及比较22项通过，72点用140／220位复核，比较覆盖19359行；另有44项对称、30项来源、404项几何及24项补充审阅检查。检查互有重叠，不累加为物理证据。下一步先确定时钟与物质的边界作用量、横向稳定来源和同边界接触，再完成同阶匹配；没有新观测、宇宙ODE或正式正文改动。
- **[有效作用量匹配与未知系数预算](experiments/eft_matching_v01/README.md)**／[结果报告](experiments/eft_matching_v01/reports/results_cn.md)：承接几何保护，明确算符与环阶、规范辅助分量阈值，完成所选带电部门的 RG 对数消除，并计算未知 Wilson 系数的条件敏感度及轻重混合拓扑诊断。12组质量／相位配置各使用1025个旧 χ 坐标；300项主检查、122项混合标量检查及34项规范检查通过。三个系数的响应满足 $K_c+2K_{\beta^2}=K_\nu$，只有两个独立形状，因此逐项10%预算不是相互独立的物理界。完整 $O(Q^2)$ 作用量及全部超引力圈图仍未完成，上一轮的 $2.5793\times10^{-21}$ 小反馈不能推广到所有量子项。该阶段将用明确对称性或UV完成固定并保护有限系数列为下一步，上述空间隔离候选已落实一个有限来源，完整混合玻费、辅助场和动能匹配仍待完成；86页正文未改，没有新观测拟合。
- **[几何保护与模场稳定的深入检验](experiments/noscale_protection_v01/README.md)**／[结果报告](experiments/noscale_protection_v01/reports/results_cn.md)：理想 no-scale 几何能取消主要隐藏反馈，但自由模场尚未稳定；指定超势稳定会恢复带电双线性分裂。另一明确的四次 Kähler 稳定候选在局部保留抵消，同时使两模场方向变重；新模场标量的一圈反馈及真空能重调引入的带电软项已分别纳入局部压力测试。873组主扫描、5238组方案比较各复用1025个旧背景点，66项主检查及52、70、35、24项专题核验通过。[独立数值复核](experiments/noscale_protection_v01/reports/independent_results_cn.md)汇总527/527通过，另列108个220位完整势点的540/540检查通过；两种计数不相加，也不构成物理证明。示例 $a=b=1,m_G=1$ eV 下，重调方案中已选各项的受力模之和相对旧树力最大约为 $2.5793\times10^{-21}$。同一匹配条件的10%局部预算约在 $m_G=3.16268\times10^5$ eV 触及；这是方案依赖诊断，非粒子质量观测界或完整高阶量子保护。下一步为模场圈图对带电二阶项、规范动能及反项的同阶匹配和真实物质读数，并检查几何参数的技术自然性；没有新背景积分、观测拟合或黎曼微观推导，86页正文未改。 该阶段提出的同阶匹配任务，现由上述[有效作用量实验](experiments/eft_matching_v01/reports/results_cn.md)完成限定部分，原数值与适用边界保持不变。
- **[隐藏破缺传递与有效理论范围](experiments/hidden_transfer_v01/README.md)**／[结果报告](experiments/hidden_transfer_v01/reports/results_cn.md)：在指定相加型 nilpotent 隐藏区中重算树级交叉项与带电 $d,B$。冻结旧背景的三相位扫描下，固定有限匹配的 10% CW 力预算约要求 $m_G\lesssim1.99396\times10^{-17}$ eV，而 VA 尺度标志达到 100 GeV 约需 $m_G\gtrsim2.37105\times10^{-6}$ eV，未找到共同窗口；后者只是必要量级诊断，界外的 100 GeV 圈图是形式延拓。正交相位的重横向谷底仍保留指数形状、系数为 $b-1/4=0.74985$，因此不是一般树级排除。35项主检查、2015项独立数值核验、两组29和24项代数检查，以及另行声明边界的9项规范传递核验通过；都是实现核验。没有新宇宙 ODE、新 $\alpha$ 曲线或观测拟合，86页正文未改。该阶段提出的明确保护完成，现由上述[几何保护实验](experiments/noscale_protection_v01/reports/results_cn.md)进一步检验；本相加型模型的原结果与适用边界保持不变。
