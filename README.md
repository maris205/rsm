...
**最新研究（2026-09-26，阈值与链保护）：** [threshold_rg_v01](experiments/threshold_rg_v01/README.md)已在固定旧有限边界下完成所选刚性重／轻部门的一圈阈值匹配：大对数移入运行系数，物理反馈不因改计算尺度而消失。2 TeV、127站点、$m_G=10^{-15}$ eV 例子的新增树加所选一圈反馈为原驱动力的 $2.00086\times10^{-5}$，即 **0.00200086%**。圈／新增树修正仍为1.234，但实际轻场顶点的大对数展开参数约 $3.63\times10^{-23}$，不能把前一比值等同于后续圈失控。保持所选交叉耦合量级的条件也已量化：指定端点泄漏与质量旁路需分别控制在约 $10^{-61}$、$10^{-60}$；这不是已计算出的辐射下限。具体矢量型Higgs链能产生目标质量矩阵，却留下一个未被矢量吸收的平坦手征模，已有体模量的远端接入仍无保护。**下一步是在该场内容中稳定剩余模，计算实际诱导耦合与旁路，再做完整局部真空匹配。** [结果报告](experiments/threshold_rg_v01/reports/results_cn.md)区分这些已算条件与完整量子模型；[86页正式稿](manuscript_cn/paper.pdf)及历史实验保持原版。

**上一阶段研究（2026-09-26，中介链）：** [有能隙的中介链与完整反馈](experiments/mediator_chain_v01/README.md)已把极小端点耦合变为声明链内可计算的树级结果：$q=3,n=128$ 给出 $\tau\simeq2.26177\times10^{-61}$。但同一匹配必然保留系数为1的模量四次自作用，原 $m_G=10^{-6}$ eV、$\Lambda=1$ TeV 例子的静态反馈约为慢驱动力的 $1.43\times10^{23}$ 倍，完整局部超引力检查确认没有被原有抵消结构消去。2 TeV、127站点、$m_G=10^{-15}$ eV 的对照有较小静态响应，但所选刚性一圈／树响应约1.23，大对数尚需分阈值匹配与RG处理。**链的小系数来源推进了，量子保护和完整模型尚未成立。** 近邻结构、体模量只耦合远端及独立接触项的控制仍需具体场内容；下一步优先处理阈值/RG、局域性来源和完整真空重匹配。[结果报告](experiments/mediator_chain_v01/reports/results_cn.md)保留全部失败与限定范围；[86页正式稿](manuscript_cn/paper.pdf)和历史实验保持原版。
