...
mu, heavy, b = sp.symbols('mu heavy b', positive=True)
x = sp.Symbol('x')  # Match the assumption-free generator returned by charpoly.
for q, n in ((2, 1), (3, 2), (3, 5)):
    ...
    zero(f'cubic derivative relative to mass q={q},n={n}',
         sp.diff(O,C,2).subs(C,0)/sp.diff(O,C).subs(C,0)-sp.sqrt(S)/(sp.sqrt(2)*v))
...
def numerical(dps):
    ...
    check(f'{dps}: order-v formal lift is strongly coupled', pref > mp.mpf('1e60'))
    ...
    return dict(dps=dps,q=q,n=n,D=str(D),S=str(S),norm_w=mp.nstr(mp.sqrt(S),90),
                cubic_yukawa_if_leading_mass_equals_v=mp.nstr(pref,90),
