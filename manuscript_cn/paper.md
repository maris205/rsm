...
最后一步使用$\ln(R/R_*)=\ln(\tau/\tau_{\rm eff})+o(1)$。这段推导回答了核心问题：**已指定的逆对数平方响应能够在反作用存在时保留晚时主项。** 沿$R$进一步积分加速度界，可将式(4.7)的差值写成$bL_\tau^{-2}+O(L_\tau^{-5})$，其中$L_\tau=\ln(\tau/\tau_{\rm eff})$。它不选择出指数2：若另给定$M^2=M_\infty^2+b\chi^{-s}$、$s>0$并保持相应正能量条件，同一论证给出$bL_\tau^{-s}+O(L_\tau^{-(2s+1)})$。平方指数的理由仍来自候选动机，$\tau$也未被标定为宇宙年龄；余项证明及条件见已有技术稿。[11](#r11)
...
Taylor展开证明的是算子一致性。耦合解的收敛还需要初值逼近、稳定性、共同时间区间和一致的正则性控制。逆对数完整反馈基准固定$L=2\pi$、$v=m_0=g=1$、$b=2$、$I=100$、$R_i=1$、$R_*=e^{-2}$、$\dot R_i=0.1$，初态$q(x,0)=0.5\cos x,\dot q=0$。独立格点ODE与保留同一时钟反馈的独立Fourier连续场，在共同窗口$0\le\tau\le20$上比较如下。[11](#r11)
...
另一个固定系数基准得到二阶空间差异及首修正后近四阶差异，不能据此宣称完整反馈已达到四阶。该固定系数基准最细参考解的一项额外精度检查仍未通过：参考场变化占修正误差1.092%，超过事前1%门限；全时修正误差本身仅改变0.00781%。失败及原始分母均保留，第8章继续汇总数值证据。[11](#r11)
...
**粒子与力网格尺度的匹配检验。** 上述失败提出了一个具体的数值问题：力网格变细时，粒子采样信息并没有同时增加，CIC沉积和回插的平滑尺度却随之改变。因此，较细的力网格未必更接近同一个连续球。本轮另行冻结两个配置，使每边粒子数与力网格数分别为64/64和128/128；两例均采用三项限制同时减半的时间控制，连续初态、初始实际动量、引力核及既有标定幅度保持不变。[63](#r63) 独立初始密度积分与33个守恒质量球壳的物理半径方程提供参照；二阶轴比之外另记录四阶角向量及横向力，避免只凭球形外观判断误差。
...
这组结果支持粒子与力网格的配比影响当前受控塌缩精度，但尚不构成一般三维收敛。理想代理密度200处的核心半径仅占约2.74和5.47个力格胞；两个参考背景配置没有补齐独立平移、时间步和微小时钟配对的收敛矩阵。95项实现检查、40项连续参照检查和440项保存产物审计通过，只确认各自范围内的实现与记录一致，不能代替未过的物理精度门限。[63](#r63)
...
RSM的核心设想是，部分有效参数的缓慢演化可能来自与可见子系统共同演化的内部自由度。作者已发表的非自治映射研究为逆对数平方响应提供数值动机；它没有确定真实物质的哈密顿量，也没有把零点序号变成宇宙时间。[7](#r7), [8](#r8) 本文据此明确给定候选相互作用，再检验它能够推出什么。

第4章的共同哈密顿量给出四项相互连接的结果。第一，最近邻非线性格点在平滑长波条件下对应一个连续场方程，首个格距修正的系数由离散算子决定。第二，区域内部的键能流相消，留下边界通量与时钟交换项；场和时钟的能量变化相反，自主整体守恒。第三，均匀时钟虽随时间演化，连续场仍保留空间平移动量守恒。第四，在纯包络、固定有限格点、有限初始能量、严格正质量下界及正向时钟分支中，时钟速度趋于有限正极限，预设逆对数平方响应保留晚时主项。[11](#r11), [53](#r53)

这些结论使用同一套作用和反作用，没有在得到微观轨迹后重新拟合宏观方程的系数。完整反馈基准的格点数从64增至128和256时，相对于独立连续场的全时空相对RMS差异由 $3.46704\times10^{-3}$ 降至 $8.66708\times10^{-4}$ 和 $2.16673\times10^{-4}$，呈约二阶空间收敛；同格点时间误差另作二阶核验。固定系数实验对首格距修正得到接近四阶的空间比较，但额外参考精度的一项1%门限未通过，原记录保留。经验误差阶支持的是指定初态和有限窗口，不能替代一般连续PDE的收敛或全局正则性定理。[11](#r11)

完整系统始终自主，场子系统沿内部时钟轨迹呈现有效含时响应。晚时命题保留的是预先选定的指数；其他正指数在对应条件下也可保留自身尾律，因而没有由辛结构或反馈唯一选出平方幂。三维各向同性、相位平均的线性无质量波另给出 $p_{\rm wave}=\rho_{\rm wave}/3$，它属于附加条件下的波动推论，与正质量下界的尾律分支不同，不能认作光子或热化机制。[53](#r53)
...
若选择电磁读数，需要先固定物质与内部变量的耦合、模型时间到物理时间的标定及环境项，再让同一组参数同时承担当前钟漂移与历史光谱响应。改变耦合或时间映射应视为另一个候选，不在看到结果后任意替换。算术方向则应改善实际连续驱动及其导数的匹配，量化检出能力；一次未检出既不是等效性证明，也不能自动识别平方指数。[1](#r1), [22](#r22), [23](#r23)

宇宙学扩展另需共同作用量和物质部门。第4章保留的CMB时间线尚未成为统一物理宇宙史；正则时钟分支使用外加FLRW结构、标准引力及CAMB初谱得到后期无碰撞聚类，不能称为从算术导出引力。[57](#r57) 受控塌缩、原初冷却及匹配网格实验保留了条件物理结果和全部精度失败，尚未建立三维收敛或实际演化气体与星系形成。[62](#r62), [63](#r63) 这些开放接口与核心固定盒模型具有不同输入，须各自验证。
