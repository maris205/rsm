...
第4章保留数值初态—CMB约束—非线性续演的存档时间线，将早期集体势占优、负压和动力学退出列为暴涨候选，尚未建立统一物理宇宙史。正则时钟分支接入辐射、物质、Λ和标准引力，以外部CAMB初谱和共同初态计算有物理年龄的后期无碰撞聚类；暴涨、早期传递与星系形成尚未共同模拟。[57](#r57) 续接的受控塌缩与条件冷却计算区分连续球形结果、有限粒子精度和气体平衡近似；参考背景的主粒子配置未通过连续球精度门限，不能据此识别微小时钟响应；本轮同质量与红移所规定的相同气体状态下，冷却微物理保持相同，形成事件处的差异只来自背景历史。气体温度、密度及恒星过程仍须实际演化。[62](#r62) 随后的匹配粒子／力网格比较显示，固定物理初态下可以通过数值表示改善部分塌缩误差；事件时刻、全轨迹半径和形状仍须共同受检验，微小时钟响应的独立收敛矩阵尚未建立。[63](#r63)

长波连接可用于进一步统计约化、局域驱动和物质读出，但真实能级、量子测量及引力仍待推导。自主背景与固定盒模型的动能不同，四个观测项目也未共享作用量和参数。代码接口、来源与独立参考须继续完善，Hubble校准扩展的公开复现材料仍待补齐。速度优势可服务条件扫描与模型比较，但仍须检验同物理内容的准确度。
