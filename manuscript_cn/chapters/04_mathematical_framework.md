...
**本章提要。** 本章的核心是一个含内部时钟的自主非线性格点候选：由共同哈密顿量推导场—时钟的相反能量交换、区域能量收支、长波场方程与连续场动量守恒；在固定有限格点的正能量分支，进一步证明预设逆对数平方响应在反作用下保留晚时主项。已有独立数值比较支持指定平滑初态和有限时间窗内的连续场对应。这里“微观”指格点自由度，“宏观”指长波场与区域读数；平方指数及相互作用仍是有前作动机的输入，尚未由黎曼结构唯一导出。三维波动压力、CMB传递、宇宙时间线及引力聚类作为分别增加条件的应用讨论，逐一保留其结果与失败边界，不作为同一哈密顿量已经推出真实宇宙史的证据。

## 4.1 离散演化算符：从正则状态到辛更新

### 4.1.1 连续动力学与离散映射的分工

当前候选沿用式(0.3)的共同哈密顿量和第3章的状态。固定周期长度$L$，令$a=L/N$、$q_N=q_0$，并取$Q=(q_0,\ldots,q_{N-1},R)$、$P=(p_0,\ldots,p_{N-1},P_R)$，对应辛形式为
...
+\frac a2M^2(R)q_\ell^2+\frac{ag}{4}q_\ell^4\right].
\end{aligned}
\tag{4.1a}
$$

最近邻作用、四次势和共享的$R$是本候选的模型假设；$R$是全盒均匀的内部自由度，尚未标定为尺度因子或宇宙年龄。辛形式规定正则演化，不能独自选择这些相互作用。具体响应写为
...
最后一步使用$\ln(R/R_*)=\ln(\tau/\tau_{\rm eff})+o(1)$。这段推导回答了核心问题：**已指定的逆对数平方响应能够在反作用存在时保留晚时主项。** 沿$R$进一步积分加速度界，可将式(4.7)的差值写成$bL_\tau^{-2}+O(L_\tau^{-5})$，其中$L_\tau=\ln(\tau/\tau_{\rm eff})$。它不选择出指数2：若另给定$M^2=M_\infty^2+b\chi^{-s}$、$s>0$并保持相应正能量条件，同一论证给出$bL_\tau^{-s}+O(L_\tau^{-(2s+1)})$。平方指数的理由仍来自候选动机，$\tau$也未被标定为宇宙年龄；余项证明及条件见已有技术稿。[11](../references.md#r11)
...
Taylor展开证明的是算子一致性。耦合解的收敛还需要初值逼近、稳定性、共同时间区间和一致的正则性控制。逆对数完整反馈基准固定$L=2\pi$、$v=m_0=g=1$、$b=2$、$I=100$、$R_i=1$、$R_*=e^{-2}$、$\dot R_i=0.1$，初态$q(x,0)=0.5\cos x,\dot q=0$。独立格点ODE与保留同一时钟反馈的独立Fourier连续场，在共同窗口$0\le\tau\le20$上比较如下。[11](../references.md#r11)

| 格点数$N$ | 全时空相对场RMS差异$\epsilon_q$ | 相邻空间误差阶 |
| ---: | ---: | ---: |
| 64 | $3.46704\times10^{-3}$ | — |
| 128 | $8.66708\times10^{-4}$ | 2.00009 |
| 256 | $2.16673\times10^{-4}$ | 2.00002 |

这里$\epsilon_q=[\sum_{n,\ell}|q_{n,\ell}-\phi^{\rm ref}_{n,\ell}|^2/\sum_{n,\ell}|\phi^{\rm ref}_{n,\ell}|^2]^{1/2}$，$n$仅标记1001个共同采样时刻；没有重新调整相位、振幅或宏观参数。时间误差另以同格点独立ODE核对，最细步长0.00125的相对场误差为$2.13029\times10^{-6}$。$I=100$的场能减少约0.0596711767，时钟能增加相同大小；三个完整反馈参考的能量／功最大绝对残差不超过$2.72\times10^{-12}$模型单位。这个有限窗口支持主阶连续场对应，没有被认定已进入式(4.7)的晚时渐近区。

另一个固定系数基准得到二阶空间差异及首修正后近四阶差异，不能据此宣称完整反馈已达到四阶。该固定系数基准最细参考解的一项额外精度检查仍未通过：参考场变化占修正误差1.092%，超过事前1%门限；全时修正误差本身仅改变0.00781%。失败及原始分母均保留，第8章继续汇总数值证据。[11](../references.md#r11)
...
推导用到两个条件：线性无质量行波经过相位平均具有动能与梯度能均分；各向同性使$\langle\partial_i\phi\,\partial_j\phi\rangle=\delta_{ij}\langle|\nabla\phi|^2\rangle/d$。各向同性是指定的统计条件，未由动力学自动导出。此处$\rho_{\rm wave}$是能量密度，精确等号对应$V=0$的波动极限，区别于式(4.6)—(4.7)要求$M_\infty^2>0$的尾律分支。若$|M^2|,\,g\langle\phi^2\rangle\ll v^2\kappa^2$且$|\dot\omega|/\omega^2\ll1$，背景和振幅在平均窗口内变化缓慢，则该关系可作为近无质量、弱非线性情形的近似。于是三维标量波集合具有与辐射同形的压力关系。它既不是任意非线性状态的状态方程，也没有导出光子、黑体谱或宇宙膨胀方程。原一维模型在相同无质量波动极限下给出的是$p_{\rm wave}=\rho_{\rm wave}$。
...
**CMB六邻域迭代给出的另一条长波路径。** 以下CMB构造是采用外部系数历史的独立应用模型，不是式(4.1a)已经导出的宇宙学结论。保留它是为了展示相同离散—连续方法如何产生可检验的传播与统计读数。其线性核心使用三维周期格点及归一化算子
...
**线性情形可以精确推进二点统计。** 以下依次讨论CMB、非线性时间线和引力聚类接口；它们各自增加初态统计、势能或标准引力等输入，其结果不作为式(4.1a)核心命题成立的前提。先对式(4.14a)的同一初值$q_{-1}=q_0$，不必预先取连续极限即可定义
...
**连接物理年龄和引力聚类的最小实现。** 作为独立宇宙学候选，可以先构造一条具有明确单位的物理连接，而不把图4.3的迭代序号直接换成年龄。这里采用与自由$R$不同的正则时钟$\chi$，动能系数$F_\chi^2$为常数，势为$V_s e^{-2\chi}$。额外输入平直FLRW度规与标准引力，让辐射、无压物质、常数真空能和均匀时钟共同决定膨胀。它不是式(4.1a)的直接长波极限，下面的粒子—网格（PM）和冷却计算也以这些额外物理为条件。记$a_{\rm cos}(t_0)=1$，则
...
该设计已经将有单位的年龄、背景膨胀和引力聚类接入同一个条件计算。它仍使用外部线性初谱，未重算时钟改变的早期物质传递，也没有把原SMICA重建场识别为物质密度。[57](../references.md#r57) 下面进一步检查一个质量与初态均已指定的塌缩对象，并给出气体冷却接口。
...
新PM试验分别改变力网格、粒子数、时间步和相对网格位置；这些误差来源需要分别检查。[59](../references.md#r59) 主例使用$64^3$粒子及$256^3$力网格，另有$128^3$粒子对照。固定核心粒子标签的中位径向缩放给出$y_{\rm PM}$，同时记录实际包围计数、同调散布及三轴形状，避免将$y_{\rm PM}^{-3}$无条件当成真实球内密度。步长同时受对数尺度因子、最大粒子位移和局部动力学时间限制；本次KDK推进和状态依赖步长不构成完整粒子映射严格辛性的证明，仍需按实际误差核验。首次高密度事件与维里化是不同的判断，本轮在前者停止。
...
独立离散力核、质量与净力检查通过，不能替代粒子表示对连续球的精度检验；细化力网格也没有增加粒子采样信息。规则立方初态的二阶矩轴比可以接近1，却不能排除高阶角向误差；本轮已直接测得明显的径向散布。这些失配已出现在无时钟的参考背景，首先限制当前粒子与网格离散方案，并非观测对时钟候选的检验。因此三维结果保留为误差诊断，未据此宣布得到经收敛验证的暗物质晕，微小clock配对差也不作为已分辨的预测。下面的冷却读数只接经过独立核验的连续球形事件。[62](../references.md#r62)
...
$n_{\rm tot}$包括中性原子、离子与电子，$\mathcal L_{\rm atom}$是单位体积、单位时间的能量损失。$T_{\rm char}$由碰撞电离平衡下的$\mu(T)$自洽求得，是指定质量与半径的特征温度，不是PM已经求出的真实气体温度；内落物质也不因达到密度200就自动热化。冷却采用历史原初H/He低密度、光学薄速率，包含碰撞激发、电离、复合和自由—自由过程；没有加入外部紫外场、金属、分子或恒星反馈。[60](../references.md#r60), [61](../references.md#r61) 因而$t_{\rm cool}$只是该假设状态的瞬时散热时间，$t_{\rm cool}<t_{\rm dyn}$也不足以单独证明恒星形成。
...
背景可以改变同一红移对应的年龄，或者同一初态达到高密度的红移，从而改变沿形成历史取样的冷却条件；这里没有额外给原子冷却率乘上$\chi^{-2}$。在固定的175个质量—红移状态中，这一微观零差异对照严格成立。参考球形事件的物理质量约$8.49\times10^{12}M_\odot$，指定$T_{\rm char}\simeq4.10\times10^6$ K时的瞬时冷却时间约27.0 Gyr，是动力学时间的29.6倍。clock事件的冷却时间增加约0.0345%，来自事件红移略变，不能据此声称原子作用率改变。网格中处于$10^{4.5}$—$10^8$ K主解释温区的132个状态，有21个局部电离弛豫时间长于冷却时间，故其平衡冷却近似需要非平衡化学进一步检验；全部警示点均保留。[62](../references.md#r62)

![受控塌缩的数值精度与条件冷却](../figures/chapter04_halo_cooling_cn.png)

**图4.5　受控塌缩与气体冷却接口的结果及边界。** 左上比较连续球形方程与主PM的半径—年龄轨迹，PM使用固定标签的中位半径代理；右上保留全部六配置的事件和共同事件前半径误差，2%与3%虚线是协议中针对主配置的精度要求，其他配置按各自条款检查。左下为175个唯一质量—红移状态的瞬时冷却／动力学时间比，两背景在相同状态下共用读数；灰色斜线表示$T<10^4$ K或$T>10^8$ K，点纹标记$10^4$—$10^{4.5}$ K敏感区，叉号表示局部电离平衡警示。右下只使用两个经过独立核验的连续球形事件，未将失败的PM代理作为气体密度或温度输入。标准引力、原初气体速率和特征气体状态均为本轮规定的物理条件。[62](../references.md#r62)

**粒子与力网格尺度的匹配检验。** 上述失败提出了一个具体的数值问题：力网格变细时，粒子采样信息并没有同时增加，CIC沉积和回插的平滑尺度却随之改变。因此，较细的力网格未必更接近同一个连续球。本轮另行冻结两个配置，使每边粒子数与力网格数分别为64/64和128/128；两例均采用三项限制同时减半的时间控制，连续初态、初始实际动量、引力核及既有标定幅度保持不变。[63](../references.md#r63) 独立初始密度积分与33个守恒质量球壳的物理半径方程提供参照；二阶轴比之外另记录四阶角向量及横向力，避免只凭球形外观判断误差。

64/64配置在$a_{\rm cos}=0.510954$达到代理阈值，事件误差为2.19%，共同事件前的最大半径误差为20.3%，最大同调散布为9.24%。与采用同样步长控制的旧64/256例相比，三项误差均明显减小；同调散布低于10%门限，但事件与半径仍未达到2%和3%的要求。末态实际包围密度比为205.91，也更接近代理200。尽管二阶三轴比例几乎相等，四阶角向量$Q_4=\langle u_x^4+u_y^4+u_z^4\rangle-3/5$却由初态约0.00068变为$-0.08609$，其中$\boldsymbol u$是相对球心的单位方向；这直接说明轴比不能单独确认球形精度。

128/128配置进一步将事件误差降至0.317%，最大半径误差降至3.1266%，最大同调散布降至5.40%，末态实际包围密度比为203.33。事件与同调性达到各自要求，但半径误差仍严格高于事前3%门限；两档事件相差$-1.834\%$，也未通过1%的比较要求。径向改善没有伴随所有角向指标的单调改善：末态$|Q_4|$减小至0.06244，但核心横向／径向力的整体L2比由6.49%增至19.86%。两例均完整保存，三个逐例失败和一个跨例失败未因接近界限而改写为通过。

这组结果支持粒子与力网格的配比影响当前受控塌缩精度，但尚不构成一般三维收敛。理想代理密度200处的核心半径仅占约2.74和5.47个力格胞；两个参考背景配置没有补齐独立平移、时间步和微小时钟配对的收敛矩阵。95项实现检查、40项连续参照检查和440项保存产物审计通过，只确认各自范围内的实现与记录一致，不能代替未过的物理精度门限。[63](../references.md#r63)

![匹配粒子与力网格尺度的受控检验](../figures/chapter04_matched_pm_cn.png)

**图4.6　匹配粒子与力网格尺度的受控检验。** 左上比较新64/64、128/128及预先指定的旧64/256半步参照的半径—年龄轨迹，连续球显示至其密度比200事件。右上保留三例事件尺度因子的绝对相对误差及各自与连续球共同事件前的最大半径误差，2%／3%线是新例门限。左下显示新两例全轨迹同调散布与10%门限。右下按原始$q$分箱显示各自实际终态的径向缩放$y=r/(a_{\rm cos}q)$中位数与16—84百分位带，注明终态尺度因子，空箱留空；浅蓝区为固定统计核心。三例均用三项步长上限减半的控制，实际步长可不同；旧完整矩阵见图4.5。两档参考背景的改善不构成微小时钟响应或逆对数平方形式的验证。[63](../references.md#r63)

要进一步计算真实星系，仍需从既有总物质预算中分出气体，演化其质量、动量和能量以及非平衡化学，再加入有依据的恒星形成与反馈；不能将重子质量重复加到已经代表全物质的粒子源上。同一质量和红移也不保证实际演化后的气体状态相同；只有给定相同局部密度、温度、组成及辐射环境时，所采用的标准微物理才相同。这一层输入应与黎曼动机及逆对数响应的待推导物理来源分开。
...
本章的核心论证至此闭合：从一个共同哈密顿量，得到相反能量交换、区域收支、长波场方程和连续动量守恒，并证明预设慢响应在指定正能量分支保留晚时主项。连续方程与守恒律是熟悉的物理结构；这里的工作是把非线性格点、内部时钟、反作用及可复算的离散误差放在同一套方程中，尚未证明平方指数唯一，也未从黎曼结构导出真实相互作用。三维压力、CMB与引力应用则保持各自附加条件和精度边界。下一章据此检验候选读数与常数、光谱、膨胀史及CMB数据的连接。
