...
**当前完成：** [中文摘要](abstract.md)及两张重新绘制的[总览图](figures/captions.md)。[摘要与图的阅读版](front_matter.md)、[单栏PDF（3页）](front_matter.pdf)由构建脚本生成。第0—9章尚未在本目录逐章改写；下一步是第0章，不把章节计划计为已完成正文。
...
| [第0章 Introduction](../chapters/01_chapter_00_introduction.md) | 导论：研究构想与总体路线 | **下一章**；按原I—VIII部分逐项对应，纳入两篇已发表前作和新的证据范围。 |
| [第1章 System Anomaly](../chapters/02_chapter_01_system_anomaly.md) | 系统异常：物理问题与研究动机 | 保留常数、膨胀与真空问题；进入该章时更新来源和观测状态。 |
...
| [References](../chapters/11_references.md) | 参考文献 | 原表作为来源；已核实文献见[技术稿参考表](../manuscript_v01/references.md)，随中文章节实际引用补入。 |
...
第一步重画总览图，不运行科学实验；第二步生成摘要与两图的Markdown和单栏PDF。正文修改入口为`abstract.md`，图注在`figures/captions.md`。排版沿用已有单栏模板，依赖Python、Matplotlib、Pandoc、XeTeX及已安装字体；PDF检查使用PyMuPDF。中间文件进入未纳入版本控制的`build/manuscript_cn/`。
