...
with mp.workdps(180):
    ...
    check('fixed_scale_source_derivative',deriverr<mp.mpf('1e-80'),max_relative_error=str(deriverr),points=10,working_digits=180,tolerance='1e-80')
    check('field_scale_total_derivative',chainruleerr<mp.mpf('1e-80'),max_relative_error=str(chainruleerr),points=10,working_digits=180,tolerance='1e-80')
...
def compare_table(filename,expect):
    ...
    errs=[];scaled_errors=[];fail=[];cells=0
    for i,row in enumerate(rows):
        ex=expect(row,i)
        for key,value in ex.items():
            actual=row[key];cells+=1
            if isinstance(value,(bool,np.bool_)):
                ok=actual==str(bool(value));err=0 if ok else float('inf');scaled=err
            else:
                ...
                scaled=abs(actual-value)/(3e-11*scale+abs_tol)
                ok=math.isfinite(actual) and scaled<=1
            errs.append(err);scaled_errors.append(scaled)
            if not ok:fail.append(dict(row=i,column=key,actual=actual,expected=value,relative_error=err))
    table=dict(rows=len(rows),numeric_and_bool_cells=cells,max_relative_error=max(errs,default=0),max_tolerance_fraction=max(scaled_errors,default=0),failures=fail)
