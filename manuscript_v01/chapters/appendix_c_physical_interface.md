...
当$V_s>0$时，需要$h_{\rm cos}>1/3$。若联立引力与物质方程，还须满足背景能量份额与流体标度条件；指数势的相应标度解已有标准研究。[8](../references.md#r8) 此处只用式(C.3)—(C.4)说明候选之间的区别，不把这一背景解当作式(C.1)的另一写法，也不在当前论文中合并两者的数值约束。
