...
这里精确的对数时间历史不能直接套用到本稿有限时段的反馈轨迹。此前正则$\chi$配指数势的宇宙学候选，借助膨胀获得条件对数解[8](../references.md#r8)；本稿则采用自由$R$，令$\chi=\ln(R/R_*)$时动能系数随$\chi$变化。两者须分别求解，不能混用场方程或约束。若采用一般轨迹，应以实际响应及其导数替代式(6.2)，具体推导和条件见[附录C](appendix_c_physical_interface.md)。
